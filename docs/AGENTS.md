...
* **scripts/**: Deployment and utility scripts
  ...
  * `setup_openai_deployments.sh`: Model deployments (gpt-4.1)
...
|----------|---------|-------------|
| `AZURE_SEARCH_ENDPOINT` | *(set by `azd up`)* | Search service endpoint |
| `AZURE_AI_PROJECT_ENDPOINT` | *(set by `azd up`)* | Foundry project endpoint |
