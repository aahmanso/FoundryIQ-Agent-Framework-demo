...
if [ -z "${RESOURCE_GROUP}" ]; then
    echo "❌ AZURE_RESOURCE_GROUP not set. Run 'azd env refresh' or pass as argument."
    exit 1
fi

# Model configuration – reads from azd env or defaults to gpt-4.1
MODEL_NAME="${AZURE_OPENAI_CHAT_DEPLOYMENT:-gpt-4.1}"
MODEL_VERSION="${AZURE_OPENAI_CHAT_MODEL_VERSION:-2025-04-14}"
MODEL_FORMAT="OpenAI"
MODEL_SKU="Standard"
